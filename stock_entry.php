...
?>
 ...
 <script>
  function getstock()
  {
	var bill_no = document.getElementById('txtbillno').value;
	//alert(bill_no);
	var scriptUrl ='getbillno.php?bId='+bill_no;
	//alert(scriptUrl);
	$.ajax({url:scriptUrl,success:function(result)
		{	
		//alert(result);
		var res=result;
		document.getElementById('stockId').innerHTML=res;
		}});
  }
  function getsalerate(i)
  {  
  var salerate= document.getElementById('srate'+i).value;
  var qty= document.getElementById('txtpqty'+i).value;
  
  var cal = parseFloat(salerate) * parseFloat(qty);
  document.getElementById('stot'+i).value=cal;
  }
 </script> 
<script>
      ...
      <div class="container-fluid">
        ...
        <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title"><b>Stock Entry</b></h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              
              <form class="form-horizontal" action="stock_ins.php" method="post" enctype="multipart/form-data">
                <div class="card-body">
                  
                  <div class="form-group row">
                    <label class="col-sm-2 col-form-label">Purchase Invoice No :</label>
                    <div class="col-sm-4">
                      <select class="chosen-select form-control" id="txtbillno" name="txtbillno" onChange="getstock();">
                      <option value="0">--- Select ---</option>
                      <?php
                      include('lib/function.php');
					  $qry="SELECT id,bill_no FROM purchase_entry WHERE sts='0' AND stock_sts='0'";
					  $st=$conn->prepare($qry);
					  $st->execute();
					  $ID=$row['id'];
                ...
                <div class="table-responsive" id="stockId">
                ...
                <div class="card-footer">
                  <button type="submit" class="btn btn-primary" id="btnsub" name="btnsub" onClick="getpurchaseno()">Submit</button>
                </div>
                <!-- /.card-footer -->
              </form>
