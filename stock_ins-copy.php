...
$lastId=$conn->lastInsertId();
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$ID=$row['id'];
			if(!empty($ID))
				{
				$qq="UPDATE stock_maintain SET qty=qty+'$txtpqty[$i]',bal=bal+'$txtpqty[$i]',rate='$txtprate[$i]',sale_rate='$srate[$i]' WHERE id='$ID'";
				//echo $qq."<br>";
				$ss=$conn->prepare($qq);
				$ss->execute();
				...
				{
				$qqq="INSERT INTO stock_maintain (dt,itemId,rate,qty,bal,sale_rate)VALUES('$dt','$txtitemId[$i]','$txtprate[$i]','$txtpqty[$i]','$txtpqty[$i]','$srate[$i]')";
				//echo $qqq."<br>";
