...
function edit(id)
 {
 window.open("edit_purchase_customer_master.php?eId="+id, "_blank" ,"toolbar=yes, scrollbars=yes, resizable=no, top=20, left=10, width=900, height=650");
 }
 ...
 function del(id)
 {
	...
	if (r == true) {
	var scriptUrl ='del_purchaseparty.php?id='+id;
	//alert(scriptUrl);
	$.ajax({url:scriptUrl,success:function(result)
	{	
	alert('Record Deleted Successfully...');
	window.location.href='purchase_customer_master.php';
	}});
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Purchase Party Entry</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              <form class="form-horizontal" action="purchase_party_ins.php" method="post">
                <div class="card-body">
                  <div class="form-group row">
                    <label class="col-sm-2 col-form-label">Company Name :</label>
                    <div class="col-sm-4">
                      <input type="text" class="form-control" id="txtconpanynm" name="txtconpanynm" placeholder="Company Name" required onBlur="ChangeCase(this)">
                    </div>
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-4">
                      <input type="text" class="form-control" id="txtaddress" name="txtaddress" placeholder="Address" required onBlur="ChangeCase(this)">
                    </div>
                  ...
                  <div class="form-group row">
                    ...
                    <!-- <select class="form-control" id="txtstate" name="txtstate" required>
                     <option value="">Select</option>
                        ...
                        $qr="SELECT id,state_nm FROM state_master WHERE sts='0'";
                        $st=$conn->prepare($qr);
                        $st->execute();
                        while($row=$st->fetch(PDO::FETCH_ASSOC))
                        {
                        $statenm=$row['state_nm'];
                        echo '<option value="'.$statenm.'">'.$statenm.'</option>';
                        }
                        $conn=null;
                        ?>
                     </select>-->
      ...
      <div class="row">
        <div class="container-fluid ">
          <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title">Purchase Party Master</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <div class="table-responsive">
                ...
                <?php 
				include('lib/function.php');
				$qry="SELECT id,company_nm,mobile,adrs,email,gst_no,state,city_pin FROM purchase_party_details WHERE sts='0' ORDER BY id DESC";
				$st=$conn->prepare($qry);
				$st->execute();
				$cnt=1;
				while($row=$st->fetch(PDO::FETCH_ASSOC))
